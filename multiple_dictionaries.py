...
from presentation.text_to_speech import TextToSpeech
...
class Recognizer(AudioSource):
    def __init__(self):
        ...
        self.dynamic_energy_ratio = 1 # ideal - 1.5 for mac
        self.pause_threshold = 0.7  # seconds of non-speaking audio before a phrase is considered complete
        self.operation_timeout = None  # seconds after an internal operation (e.g., an API request) starts before it times out, or ``None`` for no timeout

        self.phrase_threshold = 0.3  # minimum seconds of speaking audio before we consider the speaking audio a phrase - values below this are ignored
        self.non_speaking_duration = 0.5  # seconds of non-speaking audio to keep on both sides of the recording - ideally 0.5
    ...
    def recognize_sphinx(self, audio_data, dictionary, language="en-US", keyword_entries=None, show_all=False):
    	global k
        ...
        dict_path = '/home/pi/Desktop/dictionaries/' + dictionary + '.dic'
        # create decoder object
        config = pocketsphinx.Decoder.default_config()
        config.set_string("-hmm", '/home/pi/Desktop/en-in-adapt')  # set the path of the hidden Markov model (HMM) parameter files
        config.set_string("-lm", '/home/pi/Desktop/en-us.lm.bin')
...
def mainn():
	global k
	t2s.mitra_speak("activate me by saying hey there hello")
	while True:
	# obtain audio from the microphone
	   r = Recognizer()

	   with Microphone() as source:
		   print("Activate by saying, hey there hello!")
		   audio = r.listen(source)
	   if k ==0 :
		  dictionary = 'hello'
	   if k==1:
		  dictionary = 'alphabets1'
	   if k==2:
		  dictionary = 'hello1'
	
	   # recognize speech using Sphinx
	   try:
		   result = r.recognize_sphinx(audio,dictionary)
		   print result
		  
		   if ('hi' in result)  :
			   t2s.mitra_speak("Hello there, i am listening, ask me about next or ongoing events")
			   k=k+1
			   result=''
		   if ('next' in result):
			   t2s.mitra_speak("Information about upcoming events" ) 
			   
			   result=''
		   if (('now' or 'ongoing') in result):
			   t2s.mitra_speak(" say ' information about ongoing events '" ) 
